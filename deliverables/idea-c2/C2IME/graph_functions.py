...
def load_jsonl_to_graph(anotated_filename):
  ...
  for r in relations:
      ...
      if r[1]=='type_of':
        ...
        g.add((subj, typ, obj))
      # print('sujeito->r[0]: ', r[0])
      # print('sujeito: ', subj)
      # print('predicado->r[1]: ', r[1])
      # print('predicado: ', typ)
      # print('objeto->r[2]: ', r[2])
      # print('objeto: ', obj)
  return g
...
def load_files_to_graph(terms_file, relations_file, text_file):
  ...
  # print('relations_file',relations_file)
...
def graph_query(g, p_prefix, p_select, p_where, p_orderby):
  ...
  for row in qres:
    ...
    #print(f"{row.s} {row.p} {row.o} {row.docs}")
...
def run(g):
    ...
    new_graph = fgr._rule_basic_rule_entity(g, new_graph)
...
def select_grafo3(g, p_sql):
  ...
  qres = g.query(query)
  for row in qres:
    suj, pred, obj = row.opcj, row.p, row.level
    #print(f"{row.s} {row.p} {row.o}")
    #print("t1:", suj, pred, obj)
    graph_dict = {'suj':suj, 'pred': pred, 'obj':obj}
    graph_list.append(graph_dict)
    

    # suj, pred, obj = row.level, row.p1, row.docs
    # graph_dict = {'suj':suj, 'pred': pred, 'obj':obj}
    # graph_list.append(graph_dict)

    # suj, pred, obj = row.docs, row.p2, row.t_docs
    # graph_dict = {'suj':suj, 'pred': pred, 'obj':obj}
    # graph_list.append(graph_dict)

    suj, pred, obj = row.org, row.p3, row.level
    graph_dict = {'suj':suj, 'pred': pred, 'obj':obj}
    graph_list.append(graph_dict)

    suj, pred, obj = row.level, row.p4, row.slevel
    graph_dict = {'suj':suj, 'pred': pred, 'obj':obj}
    graph_list.append(graph_dict)

    # suj, pred, obj = row.s, row.applied_to, row.levels
    # graph_dict = {'suj':suj, 'pred': pred, 'obj':obj}
...
def cria_grafo_from_model(entities, relations):
  #entities, relations = ff.input_anotated_jsonl_to_knowledge(anotated_filename)
  g = create_basic_graph()
  #print(entities)
  entities = list(set([frdf.rdf_term_codification(e) for e in entities]))
  ...
  for e in entities:
      ent = e
      ent = ent.replace("'","")
      ent = URIRef(str(cnt + frdf.rdf_term_codification(ent)))
  ...
  #     print('entidade:', i)
  #     print('sujeito: ', ent)
  #     print('predicado: ', RDF.type)
  #     print('objeto: ', c2rm.entity)
  #     print('----------término -----------------')
  #     i=i+1
  # print('qtde de entidades:',i)

  # como relations é uma lista de listas, eu tive que quebrar em duas listas pra acessar os valores das triplas
  x=[]
  for c in relations:
    x.append(c.split(','))
  ...
  for r in x:
      ...
      subj = URIRef(str(cnt + frdf.rdf_term_codification(subj)))
      typ = URIRef(str(c2rm + frdf.rdf_term_codification(typ)))
      obj =  URIRef(str(cnt + frdf.rdf_term_codification(obj)))
