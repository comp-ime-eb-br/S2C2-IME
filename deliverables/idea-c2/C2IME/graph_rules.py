...
def _apply_general_triple_rule(triple, graph):
    ...
    # subj_term = subj_term.rstrip()
    # if subj_term!="http://www.w3.org/2000/01/rdf-schema#Class":
    subj_triple = (
                    subj_term, 
    ...
    if triple not in graph:
        graph.add(triple)
    # print('subj_triple: ', subj_triple)
    # print('obj_triple: ', obj_triple)
    return graph
...
def _rule_basic_rule(graph, new_graph, predicate_to_search, predicate_to_graph):
    ...
    # fim do if do bag
        ...
        if (texto=="http://idea-c2.org/c2rm/#type_of"):
          new_triple = (
                URIRef(f'{rdf_functions.rdf_term_codification(triple[2])}'),
                predicate_to_graph,
                URIRef(f'{rdf_functions.rdf_term_codification(triple[0])}')
          )
          # print('new_triple  - type_of = : ', new_triple)
        else:
          # if (triple[2]=="http://www.w3.org/2000/01/rdf-schema#class"):
          #   triple[2]=="http://www.w3.org/2000/01/rdf-schema#Class"
          new_triple = (
                URIRef(f'{rdf_functions.rdf_term_codification(triple[0])}'),
                predicate_to_graph,
                URIRef(f'{rdf_functions.rdf_term_codification(triple[2])}')
          )
          # print('new_triple = : ', new_triple)     
        new_graph = _apply_general_triple_rule(new_triple, new_graph)
        # print('triple4: início----------------')
        # print('new_triple 4: ', new_triple)
        # print('triple4: FIM----------------')           
...
def rule_entity(graph, new_graph):
    return _rule_basic_rule(graph, new_graph, 
                                predicate_to_search=URIRef("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"), 
                                predicate_to_graph=URIRef("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"))

def _rule_basic_rule_entity(graph, new_graph):
    #triples = graph_functions.search_predicate_in_graph(graph, predicate_to_search=predicate_to_search)
    triples = graph_functions.return_triples(graph)
    ...
    for triple in triples:
      ...
      pred = triple[1]
      obj = triple[2]	
      new_triple = (suj, pred, obj)
      # print('triple: início ----------------')
      # print('new_triple = : ', new_triple)
      # print('triple: término ---------------')
      new_graph = _apply_general_triple_rule_principal(new_triple, new_graph)
    return new_graph

def _apply_general_triple_rule_principal(triple, graph):
    subj_term, pred_term, obj_term = triple[0],  triple[1], triple[2]
