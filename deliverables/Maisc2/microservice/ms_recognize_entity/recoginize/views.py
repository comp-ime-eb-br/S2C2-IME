...
def recognize_load_nlp_maisc2(args):
    ...
    doc_message = word_tokenize(args) #nlp_maisc2(args) 
...
def recognize_get_entities_c2_message(args):
    ...
    for __, _ in enumerate(doc_message):
        ...
        if word in list_categ_ents_c2[0]: 
            message_ents.append(f"{_} [ACT]")#(f"{_.text} [ACT]")
            dic[__]={'value':f' {_} [ACT]', 'bgcolor':'#00FFFF', 'color':'#000'}
            count_ents+=True
        elif word in list_categ_ents_c2[1]: 
            message_ents.append(f"{_} [AGT]")
            dic[__]={'value':f' {_} [AGT]', 'bgcolor':'#F1C40F', 'color':'#000'}
            count_ents+=True
        elif word in list_categ_ents_c2[2]: 
            message_ents.append(f"{_} [DRT]")
            dic[__]={'value':f' {_} [DRT]', 'bgcolor':'#5d5dd9', 'color':'#fff'}
            count_ents+=True 
        elif word in list_categ_ents_c2[3]: 
            message_ents.append(f"{_} [WEP]")
            dic[__]={'value':f' {_} [WEP]', 'bgcolor':'#E74C3C', 'color':'#fff'}
            count_ents+=True 
        elif word in list_categ_ents_c2[4]: 
            message_ents.append(f"{_} [UNT]")
            dic[__]={'value':f' {_} [UNT]', 'bgcolor':'#3498DB', 'color':'#fff'}
            count_ents+=True 
        elif word in list_categ_ents_c2[5]: 
            message_ents.append(f"{_} [VHC]")
            dic[__]={'value':f' {_} [VHC] ','bgcolor':'#9B59B6','color':'#fff',}
            count_ents+=True
        elif word in list_categ_ents_c2[6]: 
            message_ents.append(f"{_} [PLC]")
            dic[__]={'value':f' {_} [PLC]', 'bgcolor':'#1ABC9C', 'color':'#fff'}
            count_ents+=True
        else: 
            message_ents.append(f"{_}")
            dic[__]={'value':f'{_}', 'bgcolor':'rgba(255, 255, 255, .0)', 'color':'rgb(5, 242, 250)'}
